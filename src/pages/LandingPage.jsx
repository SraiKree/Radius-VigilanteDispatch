...
/**
 * LandingPage - Entry page with branding and login
 */
function LandingPage() {
    ...
    const [selectedRole, setSelectedRole] = useState('agent'); // 'agent' | 'hq'

    // Signup modal state
    const [isSignupOpen, setIsSignupOpen] = useState(false);
    const [signupRole, setSignupRole] = useState('user'); // 'user' | 'admin'
    const [signupForm, setSignupForm] = useState({ fullName: '', email: '', password: '', department: '' });
    ...
    if (isInitialLoading) {
        return (
            <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center text-zinc-50 relative overflow-hidden z-[100]">
                {/* Background effects */}
                ...
                <div className="flex flex-col items-center gap-6 relative z-10">
                    <motion.div
                        ...
                        className="w-16 h-16 rounded-2xl bg-gradient-to-br from-red-500 to-red-700 flex items-center justify-center shadow-lg glow-danger"
                    >
                    ...
                    >
                        ...
                        <p className="text-zinc-500 font-mono text-xs uppercase tracking-widest animate-pulse">Initializing Secure Terminal...</p>
                    </motion.div>
                </div>
            </div>
        );
    ...
    return (
        <div className="min-h-screen bg-zinc-950 text-zinc-50 relative overflow-hidden">
            ...
            {/* 
            <div className="absolute inset-0 tactical-grid opacity-20 pointer-events-none z-0" />
            <div className="absolute inset-0 bg-gradient-to-br from-zinc-950/80 via-zinc-950/50 to-zinc-950/80 pointer-events-none z-0" />
            <div className="absolute inset-0 scanlines opacity-30 pointer-events-none z-0" /> 
            */}
            ...
            <div className="relative z-10 min-h-screen flex">
                ...
                <div className="flex-1 flex flex-col justify-center px-12 lg:px-24">
                    ...
                    >
                        <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-red-500 to-red-700 flex items-center justify-center shadow-lg glow-danger">
                            <Radius className="w-7 h-7 text-white" />
                        </div>
                        <div>
                            <h1 className="text-3xl font-bold tracking-tight">RADIUS</h1>
                            <p className="text-xs text-zinc-500 font-mono uppercase tracking-widest">
                                Response & Emergency Dispatch Zone
                            </p>
                        </div>
                    </motion.div>
                    ...
                    >
                        <h2 className="text-5xl lg:text-6xl font-bold leading-tight mb-4">
                            <FadeContent blur={true} duration={1000} ease="ease-out" initialOpacity={0} className="inline-block">
                                <span className="text-zinc-50">Campus Safety,</span>
                            </FadeContent>
                            <br />
                            <Shuffle
                                text="REIMAGINED"
                        ...
                        <p className="text-lg text-zinc-400 max-w-md">
                            Hyper-local emergency dispatch platform. Real-time responder tracking.
                            Transparent charity bounties.
                        </p>
                    </motion.div>
                    ...
                    >
                        {[
                            { icon: MapPin, label: 'Live Tracking', desc: 'Real-time locations' },
                            { icon: Zap, label: 'Instant SOS', desc: '< 3s response time' },
                            { icon: Users, label: 'Vigilante Network', desc: 'Campus responders' },
                            { icon: SearchCheck, label: 'Transparent Assistance', desc: 'Verified aid delivery' },
                        ].map((feature, i) => (
                            <motion.div
                                key={feature.label}
                                initial={{ opacity: 0, scale: 0.9 }}
                                ...
                                className="flex items-start gap-3 p-3 rounded-xl bg-zinc-900/50 border border-zinc-800/50 transition-all duration-300 cursor-pointer hover:bg-gradient-to-br hover:from-red-500/20 hover:to-red-700/10 hover:border-red-500/30 hover:shadow-[0_0_20px_rgba(239,68,68,0.4)]"
                            >
                                <feature.icon className="w-5 h-5 text-red-400 mt-0.5" />
                                <div>
                                    <p className="font-medium text-sm text-zinc-50">{feature.label}</p>
                ...
                <div className="flex-1 flex items-center justify-center p-6 relative">
                    ...
                    >
                        ...
                        {/* Login Block Content */}
                        <div className="bg-zinc-900 border border-zinc-800 shadow-2xl z-10 w-full space-y-6 rounded-2xl p-6 lg:p-10 backdrop-blur-md relative">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="text-zinc-50 text-xl font-bold tracking-tight">RADIUS</h2>
                            </div>

                            <div>
                                <h3 className="text-zinc-50 mb-1.5 text-2xl font-semibold">Login Portal</h3>
                                <p className="text-zinc-400">Emergency Response and Assistance Terminal</p>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-3 sm:gap-4">
                                <button
                                    ...
                                    className={cn(
                                        'btn grow h-12 rounded-xl gap-2 transition-all duration-200',
                                        selectedRole === 'agent'
                                            ? 'bg-zinc-700 border-zinc-500 text-zinc-50 shadow-inner'
                                            : 'btn-outline border-zinc-700 hover:bg-zinc-800 text-zinc-400'
                                    )}
                                >
                                ...
                                <button
                                    ...
                                    className={cn(
                                        'btn grow h-12 rounded-xl gap-2 transition-all duration-200',
                                        selectedRole === 'hq'
                                            ? 'bg-red-600/20 border-red-500/70 text-red-400 shadow-[0_0_12px_rgba(239,68,68,0.25)]'
                                            : 'btn-outline border-zinc-700 hover:bg-zinc-800 text-zinc-400'
                                    )}
                                >
                            ...
                            {selectedRole === 'hq' && (
                                <motion.div
                                    ...
                                    animate={{ opacity: 1, y: 0 }}
                                    className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/20 text-xs text-red-400 font-mono"
                                >
                                    <span className="w-1.5 h-1.5 rounded-full bg-red-400 animate-pulse" />
                                    HQ Command Mode — Admin privileges will be granted
                                </motion.div>
                            )}
                            ...
                            <div className="space-y-4">
                                <form onSubmit={handleLogin} className="mb-4 space-y-4">
                                    <div>
                                        <label className="label-text text-zinc-400 mb-2 block" htmlFor="userEmail">Operator Email*</label>
                                        <input
                                            type="email"
                                            placeholder="operator@radius.dispatch"
                                    ...
                                    <div>
                                        <label className="label-text text-zinc-400 mb-2 block" htmlFor="userPassword">Access Key*</label>
                                    ...
                                    <div className="flex items-center justify-between gap-y-2">
                                        <div className="flex items-center gap-2">
                                            <input type="checkbox" className="checkbox checkbox-primary border-zinc-700 rounded bg-zinc-800 checked:bg-red-500" id="rememberMe" />
                                            <label className="label-text text-zinc-400 p-0 text-sm cursor-pointer" htmlFor="rememberMe">Remember Terminal</label>
                                        </div>
                                ...
                                <p className="text-zinc-500 mb-4 text-center text-sm">
                                    New on our platform?{' '}
                                    <button
                                        type="button"
                                        onClick={() => { setIsSignupOpen(true); setSignupError(''); setSignupSuccess(''); }}
                                        className="text-red-500 hover:text-red-400 transition-colors bg-transparent border-none cursor-pointer"
                                    >
                                        Request Clearance
                                    </button>
                                </p>
                                <div className="divider before:bg-zinc-800 after:bg-zinc-800 text-zinc-600 text-[10px] uppercase tracking-widest font-mono">Secure Auth Gateway</div>
                                <button className="btn btn-text hover:bg-zinc-800/50 w-full h-12 rounded-xl text-zinc-400 gap-3">
                                    <img
            ...
            <Modal isOpen={isSignupOpen} onClose={() => setIsSignupOpen(false)}>
                <form onSubmit={handleSignup}>
                    <ModalHeader>
                        <ModalTitle>Request Clearance</ModalTitle>
                    </ModalHeader>
                    <ModalBody className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className={cn(
                                        'flex-1 h-11 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-all duration-200 border',
                                        signupRole === 'user'
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full h-11 px-4 bg-zinc-800/50 border border-zinc-700 rounded-xl text-sm text-zinc-50 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-red-500/50"
                                required
                            />
                        </div>
                        ...
                        {signupSuccess && (
                            <motion.div
                                initial={{ opacity: 0, y: -4 }}
                                animate={{ opacity: 1, y: 0 }}
                                className="flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-sm text-emerald-400"
                            >
                                {signupSuccess}
                            </motion.div>
                        )}
                    ...
                    <ModalFooter>
                        ...
                        <button
                            ...
                            className="btn bg-red-600 hover:bg-red-700 text-white border-none h-11 px-6 rounded-xl font-medium shadow-lg shadow-red-900/20 transition-all flex items-center justify-center gap-2"
                        >
                            {isSigningUp ? (
                                <>
                                    <span className="loading loading-ring loading-sm"></span>
